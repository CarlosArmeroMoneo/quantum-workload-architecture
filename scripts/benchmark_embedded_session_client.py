...
from aqs.embedded_session_client import PersistentSession  # noqa: E402
...
GATE_S_REFERENCE_SUMMARY = "artifacts/session_runner/ovh_session_runner_prototype_v1/summary.json"
DEFAULT_OUTDIR = "artifacts/embedded_session/ovh_embedded_session_client_v1"
BENCHMARK_REPEATS = 5
...
def _render_sequence_summary_markdown(rows: list[dict[str, Any]]) -> str:
    lines = ["# OVH Embedded Session Sequence Summary", ""]
    for row in rows:
        lines.append(
...
def _render_summary_markdown(summary: dict[str, Any]) -> str:
    lines = [
        ...
        "",
        f"- Gate S reference: `{summary['gate_s_reference_summary_path']}`",
        f"- Request rows: `{summary['request_row_count']}`",
    ...
    for row in summary["same_workload_medians"]:
        lines.append(
            ...
            f"`{row['session_runner_existing_worker_ms']:.3f}` | `{row['embedded_session_existing_worker_ms']:.3f}` | "
            f"`{row['embedded_session_autospawn_temp_worker_ms']:.3f}` |"
        )
    lines.extend(
        [
            "",
            "## Mixed Session",
            "",
            f"- Existing worker mixed per-request median ms: `{summary['mixed_existing_per_request_ms']:.3f}`",
            f"- Autospawn mixed per-request median ms: `{summary['mixed_autospawn_per_request_ms']:.3f}`",
...
def _prime_existing_worker(*, client: PersistentExecutorClient, base_request_material: dict[str, Any]) -> None:
...
def _run_persistent_warm_cli_mode(
    *,
    ...
    with tempfile.TemporaryDirectory(prefix="aqs_embedded_cli_") as tempdir:
...
def _run_session_runner_existing_mode(
    *,
    base_session_manifest: dict[str, Any],
    request_sequences: list[tuple[str, list[str]]],
    outdir: Path,
    ...
    with tempfile.TemporaryDirectory(prefix="aqs_embedded_runner_") as tempdir:
        socket_path = Path(tempdir) / "worker.sock"
        with PersistentWorkerProcess(socket_path):
            client = PersistentExecutorClient(socket_path, timeout_s=30.0)
...
def _run_embedded_session_mode(
    *,
    mode: str,
    base_session_manifest: dict[str, Any],
    request_sequences: list[tuple[str, list[str]]],
    ...
    def run_one_session(socket_path: str | Path, sequence_label: str, benchmark_repeat: int, request_ids: list[str]) -> tuple[dict[str, Any], list[dict[str, Any]], list[dict[str, Any]]]:
        ...
        with session:
            request_lookup = _request_lookup(base_session_manifest)
            for index, request_id in enumerate(request_ids, start=1):
                request = dict(request_lookup[request_id])
                session.execute_bundle(
                    workload_manifest=str(REPO_ROOT / request["workload_manifest"]),
                    plan_bundle=str(REPO_ROOT / request["plan_bundle"]),
                    system_manifest=str(REPO_ROOT / base_session_manifest["system_manifest"]),
                    execution_intent=base_session_manifest["execution_intent"],
                    objective=base_session_manifest["objective"],
                    probe_strategy=base_session_manifest["probe_strategy"],
                    planner_budget=base_session_manifest["planner_budget"],
        ...
        return session.summary(), session.trace_rows, session.health_rows

    if spawn_temp_worker:
        for benchmark_repeat in range(1, BENCHMARK_REPEATS + 1):
            for sequence_label, request_ids in request_sequences:
                socket_path = Path(tempfile.gettempdir()) / (
                    f"aqs-embedded-{benchmark_repeat}-{abs(hash((mode, sequence_label, benchmark_repeat))) & 0xFFFFFF:x}.sock"
                )
                summary, trace_rows, session_health = run_one_session(socket_path, sequence_label, benchmark_repeat, request_ids)
                ...
                start_status = next((row for row in session_health if row["label"] == "before_first_request"), {})
                end_status = next((row for row in reversed(session_health) if row["label"] == "after_session"), {})
                sequence_rows.append(
                    _session_summary_row(
                        mode=mode,
                        sequence_label=sequence_label,
    ...
    with tempfile.TemporaryDirectory(prefix="aqs_embedded_session_") as tempdir:
        socket_path = Path(tempdir) / "worker.sock"
        with PersistentWorkerProcess(socket_path):
            client = PersistentExecutorClient(socket_path, timeout_s=30.0)
            materials = _prepare_request_materials(base_session_manifest)
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Benchmark the OVH embedded persistent session client")
    ...
    runner_rows, runner_sequences, runner_health = _run_session_runner_existing_mode(
        base_session_manifest=base_session_manifest,
        request_sequences=request_sequences,
        outdir=outdir,
    )
    embedded_existing_rows, embedded_existing_sequences, embedded_existing_health = _run_embedded_session_mode(
        mode="embedded_session_existing_worker",
        base_session_manifest=base_session_manifest,
        request_sequences=request_sequences,
        outdir=outdir,
        spawn_temp_worker=False,
    )
    embedded_autospawn_rows, embedded_autospawn_sequences, embedded_autospawn_health = _run_embedded_session_mode(
        mode="embedded_session_autospawn_temp_worker",
        base_session_manifest=base_session_manifest,
        request_sequences=request_sequences,
        outdir=outdir,
    ...
    per_request_rows = cli_rows + runner_rows + embedded_existing_rows + embedded_autospawn_rows
    sequence_rows = cli_sequences + runner_sequences + embedded_existing_sequences + embedded_autospawn_sequences
    health_rows = cli_health + runner_health + embedded_existing_health + embedded_autospawn_health
        ...
        same_workload_medians.append(
            {
                "request_id": request_id,
                "workload_manifest": workload_manifest,
                "persistent_warm_cli_ms": _mode_workload_median(per_request_rows, "persistent_warm_cli", workload_manifest),
                "session_runner_existing_worker_ms": _mode_workload_median(per_request_rows, "session_runner_existing_worker", workload_manifest),
                "embedded_session_existing_worker_ms": _mode_workload_median(per_request_rows, "embedded_session_existing_worker", workload_manifest),
                "embedded_session_autospawn_temp_worker_ms": _mode_workload_median(per_request_rows, "embedded_session_autospawn_temp_worker", workload_manifest),
            }
        )

    mixed_existing_rows = [
        row for row in per_request_rows
        if row["mode"] == "embedded_session_existing_worker" and row["sequence_label"] == "mixed_trio_x6"
    ]
    mixed_autospawn_rows = [
        row for row in per_request_rows
        if row["mode"] == "embedded_session_autospawn_temp_worker" and row["sequence_label"] == "mixed_trio_x6"
    ]
    ...
    health_summary["worker_execute_median_s"] = _median(
        [
            ...
            if row["mode"] in {"embedded_session_existing_worker", "embedded_session_autospawn_temp_worker"}
        ]
    )

    pass_bars = {
        "embedded_existing_within_15pct_of_session_runner": all(
            row["embedded_session_existing_worker_ms"] <= row["session_runner_existing_worker_ms"] * 1.15
            for row in same_workload_medians
        ),
        "mixed_existing_under_60ms": _median([row["cli_wall_s"] for row in mixed_existing_rows]) * 1000.0 < 60.0 if mixed_existing_rows else False,
        "mixed_autospawn_under_60ms": _median([row["cli_wall_s"] for row in mixed_autospawn_rows]) * 1000.0 < 60.0 if mixed_autospawn_rows else False,
        "worker_execute_under_120ms": all(
            ...
            < 0.120
            for mode in ("embedded_session_existing_worker", "embedded_session_autospawn_temp_worker")
        ),
        "dispatch_reply_under_5ms": all(
            ...
            < 0.005
            for mode in ("persistent_warm_cli", "session_runner_existing_worker", "embedded_session_existing_worker", "embedded_session_autospawn_temp_worker")
        ),
    ...
    summary = {
        "gate_s_reference_summary_path": _normalized(REPO_ROOT / GATE_S_REFERENCE_SUMMARY),
        ...
        "mixed_existing_per_request_ms": round(_median([row["cli_wall_s"] for row in mixed_existing_rows]) * 1000.0, 3) if mixed_existing_rows else 0.0,
        "mixed_autospawn_per_request_ms": round(_median([row["cli_wall_s"] for row in mixed_autospawn_rows]) * 1000.0, 3) if mixed_autospawn_rows else 0.0,
        "health_summary": health_summary,
        "pass_bars": pass_bars,
        "fallback_count": sum(1 for row in per_request_rows if row["fallback_used"]),
        "recommendation": (
            "embedded session client is worth productizing as the reusable local fast path; pivot next to second-platform validation"
            if pass_bars["all_passed"]
            else "embedded client did not hold the Gate S performance envelope cleanly enough; keep the fast path local and performance-only"
        ),
    }
