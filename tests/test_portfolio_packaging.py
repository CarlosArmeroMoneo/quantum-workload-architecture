...
def test_render_report_assets_emits_portfolio_svg(tmp_path):
    ...
    assert "stack/12-remote-cudaq-and-sidecar-results" in svg_text
    assert "Blocked: Remote Host Required" not in svg_text
...
def test_portfolio_release_manifest_and_checksums_are_present():
    ...
    for artifact in manifest["artifacts"]:
        ...
        assert artifact["status"] != "blocked_remote"
    ...
    assert any(line.endswith("docs/reports/assets/portfolio_status.svg") for line in checksums)
